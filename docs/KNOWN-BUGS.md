...
**重要度**: Low
**影響バージョン**: 1.0.65-1（UPDATE-001/003 修正版で確認）
...
UPDATE-001/003 で対応した「インストールコマンド文字列」「起動時通知バナーの
バージョン比較」とは別に、changelog 本文を取得・表示する経路が存在すると
みられるが、該当コード箇所は今回未特定・未調査。

### 対応しない理由（ユーザー判断・2026-07-03）

- fork（`@bash0816/copilot-termux`）側は現時点で自前の changelog を作成・維持していない
- 実際に実行される npm インストールコマンドは既に fork を正しく指しているため、
  表示文言が upstream のものであっても実害（誤ったパッケージのインストール）はない
- 影響は表示文言の不一致のみで実質的な機能上の問題ではないため、優先度 Low として
  次回のリリースサイクルで対応する

### 次にやること

- changelog 表示ロジックの箇所を特定する（UPDATE-003 と同じ `owner:"github",repo:"copilot-cli"`
  由来のAPIレスポンスを流用している可能性が高い、要確認）
- fork 側で changelog を持つ運用にするか、changelog 表示自体を無効化するかを次回 STEP2 で検討する
...
| UPDATE-004 | `/update`実行時のchangelogがupstream公式版を表示 | Low | なし | 🔜 未修正（実害小・次回対応） |
