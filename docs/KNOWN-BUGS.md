...
> **状態**: AUTH-001 修正済み・enterprise 実機確認済み。UPDATE-001・UPDATE-003 は `registerHooks()` 方式で修正・実機検証済み（2026-07-03、ユーザーTUI目視確認済み）。UPDATE-004/005（changelog表示機能）は過剰実装と判断し完全撤去済み（2026-07-03、詳細下記）。撤去後に残る「`/update`がフォーク自身のlatestの公式changelogを表示できない」制限は UPDATE-006 として記録・**Sonnet 5からSonnet 4.6へ引き継ぎ**（2026-07-03、ユーザー判断）。version表記も `1.0.68-1` → `1.0.68`（サフィックスなし）に訂正済み。詳細はUPDATE-006本文参照。
...
## UPDATE-006: `/update` が「フォーク自身のlatest」の公式changelogを表示できない（未修正・引き継ぎ）

**重要度**: Low
**発見日**: 2026-07-03
**状態**: 🔴 未修正。担当をSonnet 5からSonnet 4.6に引き継ぎ（2026-07-03、ユーザー判断）
...
| UPDATE-006 | `/update`が「フォーク自身のlatest」の公式changelogを表示できない | Low | なし | 🔴 未修正。Sonnet 5→Sonnet 4.6に引き継ぎ(2026-07-03)。正しい設計は本文参照 |
...
3. **npm publish**: UPDATE-001/003修正・UPDATE-004/005撤去（コードレビュー済み・実機app.js確認済み）完了後、
...
4. **UPDATE-004/005撤去の残作業**: 実グローバルインストールでのスモークテスト・ユーザーTUI実機確認
   （`/update`実行時にupstream公式changelogがそのまま表示されることの目視確認）→ npm publish
