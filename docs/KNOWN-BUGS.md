...
**重要度**: Low
**発見日**: 2026-07-03
**状態**: ✅ 実装済み（2026-07-04）。`patchAppJsSource()` にてDO()のlatest取得先を `@bash0816/copilot-termux` npm dist-tagに差し替え。コードレビュー（codex Conditional Go→追加修正→Go相当）・スモークテスト済み。TUI目視確認・npm publish 待ち
...
| UPDATE-006 | `/update`が「フォーク自身のlatest」の公式changelogを表示できない | Low | なし | ✅ `patchAppJsSource()`でDO()のlatest参照をフォーク自身のnpm dist-tagに差し替え済み（2026-07-04）。コードレビュー・スモークテスト済み。TUI目視確認・npm publish待ち |
...
4. **UPDATE-006 TUI目視確認**: TUI `/update` 実行時にフォーク自身のlatest（`@bash0816/copilot-termux` npm dist-tag）を基準に更新チェックし、upstream公式changelogが表示されることをユーザーが確認 → npm publish
