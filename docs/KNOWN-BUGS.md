...
| BUG-NEW-1 | Free TUI auto モードで 400 | High | MODEL-001 | ✅ glibc mode実装済み(1.0.65-2〜)・GPT-5.5コードレビュー完了(2026-07-06、Blocker4件中3件修正・1件は設計明文化で解消)。wrapperバージョンを1.0.68-1に更新(commit `96acfcf`)。実機スモークテストでLD_PRELOAD継承バグを発見・修正(commit `c570df2`)、setup成功・glibc mode起動・`-p`応答を確認済み。🔍 TUI目視確認待ち |
